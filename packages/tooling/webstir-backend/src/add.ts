...
export async function runAddRoute(options: AddRouteOptions): Promise<BackendAddResult> {
  const name = normalizeRouteName(options.name);
...
export async function runAddJob(options: AddJobOptions): Promise<BackendAddResult> {
  ...
  const packageJsonPath = path.join(options.workspaceRoot, 'package.json');
  const jobsRoot = path.join(options.workspaceRoot, 'src', 'backend', 'jobs');
  const jobDirectory = path.join(jobsRoot, name);
  ...
  await assertNoSymlinkComponents(options.workspaceRoot, jobDirectory, 'job scaffold path');
  ...
  await assertNoSymlinkComponents(options.workspaceRoot, jobsRoot, 'job scaffold path');
  try {
    await mkdir(jobDirectory);
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === 'EEXIST') {
      throw new Error(`Job '${name}' already exists.`, { cause: error });
    }
    throw error;
  }
  await assertNoSymlinkComponents(options.workspaceRoot, jobDirectory, 'job scaffold path');
  await writeFile(jobFilePath, buildJobTemplate(name), { encoding: 'utf8', flag: 'wx' });
...
function buildJobTemplate(name: string): string {
  const logLabel = JSON.stringify(`[job:${name}] ran at`);
  return `// Generated by webstir add-job
import path from 'node:path';
import { fileURLToPath } from 'node:url';

export async function run(): Promise<void> {
  console.info(${logLabel}, new Date().toISOString());
}

// Execute the built job module directly to run it once.
...
function normalizeJobName(name: string): string {
  ...
  if (
    ...
    !isPortableFilenameSegment(normalized)
  ) {
    throw new Error(
      `Invalid job name '${normalized}'. Expected a portable single path segment without '.', '..', slashes, reserved names or characters, or control bytes.`,
    );
  }
...
async function assertNoSymlinkComponents(
  ...
  if (path.isAbsolute(relative) || relative === '..' || relative.startsWith(`..${path.sep}`)) {
    throw new Error(`Refusing ${label} outside the workspace root.`);
  }
  ...
  for (const [index, segment] of segments.entries()) {
    ...
    try {
      ...
      if (isFinalTarget && finalTargetType === 'regular-file' && !stats.isFile()) {
        throw new Error(`Refusing ${label}; path is not a regular file: ${current}`);
      }
      if (!isFinalTarget && !stats.isDirectory()) {
        throw new Error(`Refusing ${label}; path component is not a directory: ${current}`);
      }
    } catch (error) {
