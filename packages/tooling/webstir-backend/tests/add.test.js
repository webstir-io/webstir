...
test('runAddRoute writes manifest metadata and patches fastify scaffolding', async () => {
  ...
  const routeFile = await fs.readFile(path.join(workspace, 'src', 'backend', 'server', 'routes', 'accounts.ts'), 'utf8');
  const fastifyFile = await fs.readFile(path.join(workspace, 'src', 'backend', 'server', 'fastify.ts'), 'utf8');
  assert.match(routeFile, /Generated by webstir-bun add-route --fastify/);
  assert.match(fastifyFile, /import \{ register as registerAccounts \} from '\.\/routes\/accounts';/);
