...
SPA now bypasses this daemon, `full` now uses a Bun-native frontend host, and `ssg` remains on this daemon-backed path intentionally for this phase. Use `webstir watch --frontend-runtime legacy` only with `ssg`.
...
- Explicit legacy SSG workflow: `webstir watch --frontend-runtime legacy`
...
- Bun-native SPA and `full` watch bypass this daemon entirely in the supported path.
