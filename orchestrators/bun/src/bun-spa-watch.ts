...
import {
  createBunSpaFallbackFetchHandler,
  createBunSpaRoutes,
  loadBunSpaEntry,
...
export async function startBunSpaFrontendWatch(
  ...
  const servedAddress = createServedAddress(host, startSpaServer(host, port, servedEntry));
  const watchers = watchSpaRegenerationTargets(paths, page, async (nextEntry) => {
    servedAddress.server.reload({
      fetch: createBunSpaFallbackFetchHandler(),
...
function startSpaServer(host: string, port: number, spaEntry: BodyInit): ReloadableServeServer {
  return Bun.serve({
    hostname: host,
    port,
    development: true,
...
function watchSpaRegenerationTargets(
